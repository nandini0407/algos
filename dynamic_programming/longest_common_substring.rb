...
# other string, and return the longest sommon suffix
# This can be represented by the recurrence relation : longest_common_substring(s1, s2) = max(longest_common_suffix(s1[0..i], s2[0..j]))
# where 0 <= i < s1.length and 0 <= j < s2.length
...
# If both letters are same, then check if the smaller problem has been solved, i.e., the case where that letter is missing from both strings
